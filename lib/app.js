...
module.exports = (tilelive, options) => {
  ...
  // Only these keys are part of the public TileJSON spec; anything else
  // (e.g. internal filenames/paths, backend-specific source config) must
  // not be forwarded to clients.
  const TILEJSON_SAFE_KEYS = [
    "name",
    "description",
    "version",
    "attribution",
  ...
  app.get("/index.json", (req, res, next) => {
    ...
    return tilelive.load(uri, (err, source) => {
      ...
      return tessera.getInfo(source, (err, info) => {
        ...
        const sanitizedInfo = {};
        TILEJSON_SAFE_KEYS.forEach((key) => {
          if (info[key] !== undefined) {
            sanitizedInfo[key] = info[key];
          }
        });

        sanitizedInfo.tiles = [tileUri];
        sanitizedInfo.tilejson = "2.0.0";
        sanitizedInfo.scheme = "xyz";

        res.set(populateHeaders({}, params, { 200: true }));
        return res.send(sanitizedInfo);
      });
    });
  });
